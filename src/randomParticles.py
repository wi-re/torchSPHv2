...
from src.simplex import getOctaveSimplexNoisePeriodic, getSimplexNoisePeriodic, _init

def createPotentialFieldPeriodic(n = 256, res = 4, octaves = 2, lacunarity = 2, persistance = 0.5, seed = 1336, kind = 'simplex'):

    if kind == 'simplex':
        x = np.linspace(0,1,n)
        y = np.linspace(0,1,n)
        sampled = getOctaveSimplexNoisePeriodic(x, y, res * 0.25, octaves, lacunarity, persistance, seed)
    elif kind == 'perlin':
