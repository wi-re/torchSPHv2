...
parser.add_argument('-m','--m', type=int, default=9)
parser.add_argument('--seed', type=int, default=42)
parser.add_argument('-d','--frameDistance', type=int, default=0)
...
model = DensityNet(fluidFeatures.shape[1], boundaryFeatures.shape[1], coordinateMapping = coordinateMapping, n = n, m = m, windowFn = windowFn, rbf_x = rbf_x, rbf_y = rbf_y, batchSize = args.cutlassBatchSize)
...
for epoch in range(args.epochs):
    ...
    if args.forwardLoss:
        trainingEpochLoss2 = processDataLoader(epoch,1, train_ds, train_dataloader, model, optimizer, False, prefix = 'forward')
